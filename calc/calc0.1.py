...
root = Tk()
root.title('Калькулятор имени меня')
...
def calculate(): #логика калькулятора
    ...
    if operation == '/':
        if operand1 == 0 or operand2 == 0: #долго думал как красиво запретить
            result = 'хорошая попытка, но не'     #нет ничего красивее обычного if-else
        else:
            result = operand1 / operand2
    if operation == '*':
        result = operand1 * operand2
...
label.grid(row=0, column=0, columnspan=4, sticky="nsew") #от размера надписи зависит размер кнопок

button = Button(root, text='CE', font=("Times New Roman", 15, "bold"), bg="#DFDDDD",
                     foreground="#F30000", command=lambda text='CE': click(text))
button.grid(row=1, column=3, sticky="nsew") #кнопки располагаются с помощью грида
for row in range(4):
    for col in range(4):
        button = Button(root, font=("Times New Roman", 15, "bold"), bg="#DFDDDD", text=buttons[row][col],
